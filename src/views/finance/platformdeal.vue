  ...
  <div class="app-container">
    ...
    <el-table
      ...
      <el-table-column fixed="right" label="操作" align="center" width="150" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button v-if="scope.row.is_canpay > 0" type="primary" size="mini" @click="handleDopay(scope.row)">去支付</el-button>
        </template>
      </el-table-column>
    </el-table>
...
<script>
import { getCompanyFinanceList, postRechargetopay } from '@/api/finance'
...
export default {
  ...
  methods: {
    getList() {
      ...
      getCompanyFinanceList(this.listQuery).then(response => {
    ...
    handleDopay(row) {
      postRechargetopay(row).then(response => {
    ...
    getRechargeStatus() {
      getParameterItem('COMPANY_RECHARGE_STATUS').then(response => {
