<template>
  <div class="amap-page-container">
    ...
    <el-amap
      ...
      :plugin="plugin"
      :zoom="zoom"
      :map-style="'amap://styles/'+map_style"
      vid="amapDemo"
      class="amap-demo">
      <el-amap-marker
    ...
    <div style="position: fixed;bottom: 20px;left: 40px;">
      <el-button size="small" style="float: left;width: 120px;background: #47A447;color:white;">空闲人员:{{ temporal_data['driver_free'] }}</el-button>
      <el-button size="small" style="float: left;width: 120px;background: #d31c16;color:white;">忙碌人员:{{ temporal_data['driver_service'] }}</el-button>
      <el-button size="small" style="float: left;width: 120px;background: #F0AD4E;color:white;">今日总收入:{{ temporal_data['total_money'] }}</el-button>
      <el-button size="small" style="float: left;width: 120px;background: #39B3D7;;color:white;">未派单:{{ temporal_data['new_nums'] }}</el-button>
      <el-button size="small" style="float: left;width: 120px;background: #D9534F;;color:white;">执行中:{{ temporal_data['service_nums'] }}</el-button>
      <el-button size="small" style="float: left;width: 120px;background: #418BCC;color:white;">已完成:{{ temporal_data['completed_nums'] }}</el-button>
      <el-button size="small" style="float: left;width: 120px;background: #999999;color:white;">销单量:{{ temporal_data['cancel_nums'] }}</el-button>
    </div>

    <div style="position: fixed;bottom: 20px;right: 20px;">
      <el-button type="primary" size="mini" @click="traffficHandle">路况</el-button>
      <el-button type="primary" size="mini" @click="changeMapStyle('normal')">标准</el-button>
      <el-button type="primary" size="mini" @click="changeMapStyle('dark')">黑夜</el-button>
      ...
      client.on('message', (topic, message) => {
        if (message.toString()) {
          ...
          if (Object.keys(data).length > 0) {
            ...
            // var res = data.data
            // if (data.code === 'driver' && res) {
            //   var is_has = false
            //   for (var i = 0; i < this.driver_status.length; i++) {
            //     if (res.status === this.driver_status[i]) {
            //       is_has = true
            //       break
            //     }
            //   }
            //
            //   for (const v of this.driver_list) {
            //     if (v.id === res.id) {
            //       const index = this.driver_list.indexOf(v)
            //       this.driver_list.splice(index, 1)
            //       break
            //     }
            //   }
            //   if (is_has) {
            //     this.driver_list.unshift(res)
            //   }
            //
            //   this.getWindowInfo()
            //   this.getDriverStatusCount()
